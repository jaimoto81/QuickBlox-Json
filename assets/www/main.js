...
function getAllUsers(token) {
	...
	var url = 'http://api.quickblox.com/users.json'
	alert('[DEBUG] Getting all users: GET ' + url );
	$.ajax({
		type: 'GET',
		url: url,
		headers: {
    	...
    	success: function(response) {
			...
			$(response.items).each(function(index, current){
				var element = '<li>[<a href="#" id="' + current.user.id + '">delete</a>] (' + current.user.id + ') ' + current.user.login + '</li>';
...
function getFBUser(token) {
	// See more documentation on the wiki -- http://wiki.quickblox.com/Users#Retrieve_API_Users_for_current_application
	var url = 'http://api.quickblox.com/users/by_facebook_id.json'
	alert('[DEBUG] Getting all users: GET ' + url );
	$.ajax({
		type: 'GET',
		url: url,
		data: 'facebook_id=780496473',
		headers: {
        	"Content-Type":"application/json",
        	"QuickBlox-REST-API-Version":"0.1.1",
        	"QB-Token":token
    	...
    	success: function(response) {
			alert('[DEBUG] Server Facebook response: ' + response.user.id);
			// Prints users list into the #users container.
			$('#fusers').html('');
				var element = '<li>[<a href="#" id="' + response.user.id + '">delete</a>] (' + response.user.id + ') ' + response.user.login + '</li>';
				$('#fusers').append(element);
			//});
		},
		error: errorCallback
	});
...
function addUser(token) {
	...
	var url = 'http://users.quickblox.com/users.json'
	var data = 'user[owner_id]=' + QB.ownerId + 
			'&user[login]=' + login + 
			'&user[password]=' + password;
	
	alert('[DEBUG] Add new user: POST ' + url + '?' + data);
	...
	$.ajax({
		...
		data: data,
		success: function(response) {
			alert('[DEBUG] User has been successfully added, server response:');
			alert(response);
...
function deleteUser(token, id) {
	...
	alert('[DEBUG] Delete user: DELETE ' + url + '?' + data);
